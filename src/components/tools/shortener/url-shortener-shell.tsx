...
export function UrlShortenerShell() {
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      if (editDraft.originalUrl.trim())
        body.originalUrl = editDraft.originalUrl.trim();
      if (editDraft.customSlug.trim() !== "")
        body.customSlug = editDraft.customSlug.trim();
      else body.customSlug = null;
      if (editDraft.expiresIn === "0") body.expiresIn = null;
      else if (editDraft.expiresIn !== "")
        body.expiresIn = Number(editDraft.expiresIn);
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      await fetch(`/api/tools/shorten?id=${deleteConfirmId}`, {
        method: "DELETE",
      });
                    ...
                    <div className="rounded-lg border border-ekd-charcoal/10 dark:border-ekd-light-gray/10 p-4 space-y-3">
                      ...
                      <div className="flex gap-2">
                        <Button
                          size="sm"
                          variant={
                            qrCenter.mode === "text" ? "default" : "outline"
                          }
                          className={
                            qrCenter.mode === "text"
                        ...
                        <Button
                          size="sm"
                          variant={
                            qrCenter.mode === "logo" ? "default" : "outline"
                          }
                          className={
                            qrCenter.mode === "logo"
                              ? "bg-ekd-gold text-ekd-charcoal"
          ...
          <Card className="p-6 border-ekd-charcoal/10 dark:border-ekd-light-gray/10">
            ...
            <ul className="space-y-2 text-sm text-ekd-charcoal/80 dark:text-ekd-light-gray/80">
              ...
              <li className="flex items-start gap-2">
                <span className="text-ekd-gold mt-0.5">•</span>
                <span>
                  Set an expiration date if you want the link to expire
                </span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-ekd-gold mt-0.5">•</span>
                          ...
                          <div>
                            ...
                            <Input
                              ...
                              onChange={(e) =>
                                setEditDraft(
                                  (d) =>
                                    d && { ...d, originalUrl: e.target.value },
                                )
                              }
                              className="text-sm"
                            />
