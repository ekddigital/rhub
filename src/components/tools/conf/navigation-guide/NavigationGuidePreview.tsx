...
function exportBasename(): string {
  return `lsuic-navigation-guide-${NAV_GUIDE_META.confYear}`;
}

export function NavigationGuidePreview() {
  ...
  useEffect(() => {
    void warmupNavigationPdfExport();
  }, []);
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      if (!pagesReady) {
        throw new Error("Navigation guide pages did not finish rendering.");
      }
  ...
  return (
    <div className="space-y-4">
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          <span style={{ fontSize: "12px", fontWeight: 600, color: C.blue }}>
            Navigation Guide Preview
          </span>
          <span style={{ fontSize: "10px", color: C.muted }}>
            {NAV_GUIDE_TOTAL_PAGES} pages · A4 printable
          </span>
        </div>
      ...
      {exportError && (
        <div className="nav-guide-no-print rounded-lg border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm text-red-700">
          {exportError}
        </div>
      )}
