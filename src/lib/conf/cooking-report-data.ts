/** Certified Cooking Committee financial report — shared by payments tool and conference report. */
...
function sumCookingLineItems(items: readonly CookingLineItem[]): number {
  return items.reduce((sum, item) => sum + item.amount, 0);
}

/** Expenditure categories derived from certified line-item records (single source of truth). */
export function buildCookingBudgetCategories() {
  const food = sumCookingLineItems(COOKING_FOOD_ITEMS);
  const seasonings = sumCookingLineItems(COOKING_SEASONINGS);
  const equipment = sumCookingLineItems(COOKING_EQUIPMENT);
  const transfers = COOKING_TRANSFERS.reduce((sum, row) => sum + row.amount, 0);

  return [
    { label: "Food, meat, vegetables, and groceries", amount: food },
    { label: "Seasonings, baking supplies, and condiments", amount: seasonings },
    { label: "Kitchen equipment and supplies", amount: equipment },
    { label: "Member reimbursements and transfers", amount: transfers },
    { label: "Transportation", amount: COOKING_TRANSPORTATION },
  ] as const;
}

export function computeCookingExpenditure(): number {
  return buildCookingBudgetCategories().reduce(
...
export function buildCookingAppendixPages(
  ...
  const pushPage = (
    plan: Omit<CookingAppendixPagePlan, "pageIndex" | "pageCount">,
  ) => {
